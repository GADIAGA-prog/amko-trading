...
export default function Pricing({ marketPrices, deals = [], onPricingSaved, initialDealId }) {
  ...
  const [qualitySpread, setQualitySpread] = useState('8.00');
  const [apiSulphurAdj, setApiSulphurAdj] = useState('0.50');
  const [traderMargin,  setTraderMargin]  = useState('1.00');
  ...
  // ── Prix de référence depuis le Dashboard (converti $/bbl → $/MT) ─
  const useRefPrice = () => {
    const key = MARKER_TO_KEY[marker] || 'brent';
    const ref = marketPrices?.[key];
    if (ref) setMarkerPrice(String((Number(ref) * (Number(bblPerMT) || 1)).toFixed(2)));
  };
  ...
  const qtyMT      = Number(quantityMT) || 0;
  const notional   = qtyMT * finalPrice;
        ...
        <Card>
          ...
          <CardBody>
            <div className="space-y-3">
              ...
              <Field label="Prix marker ($/MT)" hint="Moyenne sur la période de pricing (converti depuis le $/bbl du Dashboard via bbl/MT)">
                <div className="flex gap-2">
                  <Input type="number" step="0.01" value={markerPrice}
                    onChange={e => setMarkerPrice(e.target.value)} placeholder="Ex. 1181.75" />
                  {hasRefPrice && (
                    <Button variant="outline" size="sm" onClick={useRefPrice}
                      title="Utiliser le prix de référence du Dashboard (converti en $/MT)">
                      ↗ Réf.
                    </Button>
                  )}
                </div>
              ...
              <div className="border-t border-slate-200 dark:border-slate-700 pt-3 mt-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Field label="bbl/MT (conversion réf.)" hint="Sert à convertir le prix $/bbl du Dashboard">
        ...
        <Card>
          ...
          <CardBody>
            <div className="space-y-2 text-sm">
              ...
              <Row label="Marker"             value={fmtUSD(Number(markerPrice) || 0, 2) + '/MT'} />
              ...
              <Row label="Différentiel total" value={fmtUSD(diff, 2) + '/MT'} />
            </div>
            ...
            {linkedDeal && (
              <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                ...
                <div className="space-y-1.5 text-sm">
                  ...
                  <Row label="Prix estimé actuel" value={linkedDeal.estimatedPrice ? fmtUSD(Number(linkedDeal.estimatedPrice), 2) + '/MT' : '—'} />
                  <Row label="Prix calculé"   value={fmtUSD(finalPrice, 2) + '/MT'} />
                </div>
